...
const rawEntries: Omit<Entry, "n">[] = [
  // ── Color ────────────────────────────────────────────────────────────────
  {
    ...
    why: {
      zh: "它本身就是生成式默认值。没人选它，只是 Tailwind 示例和无数模板的出厂配色，所以一眼看去，就是「没做选择」。",
    ...
    fix: {
      zh: "选一个有理由的主色。要用渐变就让它有方向和目的，而不是拿来当「高级感」的贴纸。",
  ...
  },
  {
    ...
    what: {
      zh: "amber / stone / orange 堆出来的暖棕色调，配上柔和的米色背景。",
  ...
  },
  {
    ...
    why: {
      zh: "这套「语义色」是 Tailwind / Bootstrap 的出厂设定，没人真去挑过。几种毫不相干的糖果色挤在一起，和品牌无关，彼此也不搭，一盘默认的彩虹糖，每个提示框各喊各的调。",
    ...
    fix: {
      zh: "语义色也要从你自己那一个调色板里长出来：用同一主色的深浅加中性灰，只给少数真需要区分的状态上色。大多数「提示」根本不用颜色。",
  ...
  },
  {
    ...
    fix: {
      zh: "先用文字和字重把状态说清楚：一个加粗的「错误」，比任何颜色都先被读到。真要上色，就只用你自己调色板里一个克制柔和的强调色，衬在中性底上。屏幕上很少真的需要红黄绿三盏灯同时亮着。",
  ...
  },
  {
    ...
    what: {
      zh: "近黑的深蓝页面，顶部一团柔光，卡片的背景本身也是渐变，上浅下深。这是「高级暗色模式」的出厂设定，每个表面都在营造气氛，没有一处是实心色。",
    ...
    fix: {
      zh: "选一个实心背景色，一以贯之。表面的层次用一条细边框加克制的阴影，而不是渐变。真要有光晕，就让它指向某样东西，而不是把一切背后的空白填满。",
  ...
  },
  {
    ...
    what: {
      zh: "给一个开发者工具/SaaS 落地页套上 Playfair、Lora 这类展示衬线做正文。",
    ...
    fix: {
      zh: "界面正文用一款你选定的、好读的无衬线。衬线只在你真的想要那种嗓音时、且是正文衬线时才用。",
  ...
  },
  {
    ...
    what: {
      zh: "划掉一个词、加下划线、刷高亮，不为删改，不为标注，纯粹是当成强调的装饰。",
    ...
    why: {
      zh: "和衬线强调、高亮关键词是同一个病：模型不信任文字本身能承载重点，于是往上画记号。划掉一个并没被删的词是在「耍酷」，下划线是假重点，高亮是荧光笔滥用。（这份网站的早期版本自己就犯了，首页标题上划了线、词上刷了高亮）",
    ...
    fix: {
      zh: "重点靠字重、字号、断行和句子结构。删除线留给真的删改，下划线留给链接，高亮留给真的标注，每一样都只在它有职责时才出现。",
  ...
  // ── Copy ─────────────────────────────────────────────────────────────────
  {
    ...
    why: {
      zh: "当每个词都被强调，就没有词被强调。这是模型「不知道重点是什么，所以全标上」的视觉表现。",
  ...
  },
  {
    ...
    why: {
      zh: "这是生成文案的节奏指纹：结构永远对称、情绪永远高一度、信息永远含糊。读起来顺，但什么都没说。",
  ...
  },
  {
    ...
    what: {
      zh: "标题、按钮、每个要点前都挂着 emoji：🚀 提速、⚡ 极快、🔒 安全、🎉 好用。",
      en: "An emoji stuck on every heading, button, and bullet point: 🚀 for launch, ⚡ for fast, 🔒 for secure, 🎉 for delight.",
    },
    why: {
      zh: "emoji 是廉价的亲和力。它替代了本该由文字和设计传达的语气，让一个能读你邮件、跑你 shell 的产品显得像在讨好你。每个要点前面加一个图标并不会让它更清楚，只会更吵。",
  ...
  // ── Components ─────────────────────────────────────────────────────────
  {
    ...
    what: {
      zh: "一个表示状态（在线／就绪／直播）的圆点，画成实心圆外面套一圈浅色光晕，通常还一直呼吸／脉冲，几乎永远是饱和绿。",
    ...
    why: {
      zh: "状态本身只是个极小的信号，而光晕、脉冲、呼吸都是纯装饰，不带任何信息。每个 AI 产品都朝你「● 就绪」地闪。把一个二元状态放大成一颗发光宝石，是典型的过度设计。",
    ...
    fix: {
      zh: "一个小小的、扁平的、单色圆点加一句文字就够了。不要光晕，不要脉冲。只在状态真的重要、或真的会变时才上色。",
  ...
  },
  {
    ...
    what: {
      zh: "任何一条列表项都套一个浅色圆角方块，左边一条 4px 彩色竖条。不只是「提示/注意」，功能点、更新日志、任何列表都这么包。",
    ...
    why: {
      zh: "圆角 + 左侧彩条本来是文档 admonition（旁注）的语义组件，却被当成万能装饰，套在一切列表项上。于是每一条都长得像「重点提示」，真正的重点反而没了。这套组合本身就是文档模板的默认长相。",
    ...
    fix: {
      zh: "列表就让它是列表：靠对齐、间距和层级。旁注是稀缺资源，一页最多一两个，且只在真的是旁注时才用。",
  ...
  },
  {
    ...
    why: {
      zh: "图标砖是「看起来做了设计」的最短路径。图标往往和内容无关（一个齿轮、一个闪电、一个地球），纯粹为了填满格子。",
  ...
  },
  {
    ...
    why: {
      zh: "这是 2021 年 Dribbble 的冷冻切片。毛玻璃和满圆角是「效果预设」，套上就完事，和产品要表达什么无关。",
    ...
    fix: {
      zh: "圆角选一个值，全站一致，通常小而克制。用实心面和留白造层次，而不是模糊。",
  ...
  },
  {
    ...
    what: {
      zh: "一个小卡片或图标，投出一大团柔软的阴影，四面漫开、远超它本身：大模糊、低透明度、几乎不带偏移，像悬在一团雾里。",
    ...
    why: {
      zh: "阴影表达的是一个表面浮在下层之上有多高：真实的光会落下一小片贴地的接触阴影，再加一层柔和的环境阴影，模糊随高度变化。而这团阴影什么都不表达——罩在按钮外面的和罩在 hero 卡片外面的是同一团雾。模型抓它，是因为一大片柔软的模糊在缩略图里像「有层次」「高级」，可它对应不上任何真实高度，也不带任何信息，是拿氛围冒充高度。于是每个元素都悬在自己那团诡异的云里。",
    ...
    fix: {
      zh: "阴影是在模拟高度，就给它一套克制的高度刻度并守住：模糊要紧、偏移要小、透明度要低，绝不让阴影比投出它的东西还大。很多时候一条细边框就把层次分好了，根本不需要阴影。真要用，就拿一层贴地的接触阴影垫一层克制的环境阴影，而且别上色——带色的光晕不是层次。",
  ...
  },
  {
    ...
    what: {
      zh: "把同一个圆角值套在每一层：外框是大圆角，里面的框也是同样的大圆角，所以嵌套的角对不齐，内圈的弧和外圈的弧在每个角上都别扭地打架。",
    ...
    why: {
      zh: "嵌套圆角是有规则的：内圈半径 = 外圈半径 − 两者之间的间距，这样两条弧才会平行。AI 生成的界面跳过这步计算，给所有东西套上同一个圆角 token，于是嵌套的角永远对不上。这是「圆角是默认来的、不是量出来的」的指纹。",
    ...
    fix: {
      zh: "算一下嵌套半径：内 = 外 − 内边距。或者干脆别给内层加圆角。用一套小而克制的圆角刻度，让每个角都和它的父级同心。",
  ...
  },
  {
    ...
    why: {
      zh: "徽章本该标记状态，现在被用来制造「热闹」和「可信」的假象。数量一多，每一个都失去了信号价值。",
  ...
  },
  {
    ...
    what: {
      zh: "让模型「画个图标」，再把它给的东西直接用上：一个圆滚滚的色块加两颗点眼睛，一个用基本图形拼出来的小怪物，一个只是圆角方块加张脸的 logo。这是机器画不好、却被当成产品标识的矢量图。",
  ...
  },
  {
    ...
    what: {
      zh: "每个图标都套进一个圆角方块，方块底色是这个图标颜色的半透明版：蓝图标配淡蓝、绿图标配淡绿。这是默认的「图标容器」：text-{color} 图标加一层 bg-{color}/10。",
    ...
    fix: {
      zh: "让图标就当图标：继承文字颜色，不要容器。真需要容器时（比如一个真正的按钮），给它一个从调色板里选定的实心表面色，而不是一层跟图标同色的半透明底。",
  ...
  // ── Layout ─────────────────────────────────────────────────────────────
  {
    ...
    what: {
      zh: "全大写标签加一个数字/图标，复制成一排排等质的卡片：功能网格、仪表盘指标卡都是它。",
    ...
    why: {
      zh: "它假装信息有结构，其实只是把一堆不相关的东西塞进同样的盒子凑满一屏。全大写小标签是「看起来做过设计」的默认套路。",
    ...
    fix: {
      zh: "先想清楚最重要的一件事，把它讲透。真要并列，用真正的层级和对比，而不是一网格等权重的卡片。",
  ...
  // ── Evolved slop ─────────────────────────────────────────────────────────
  {
    ...
    fix: {
      zh: "等宽字体留给代码，别拿来当界面外壳。ASCII 艺术、终端隐喻要为产品服务才用。真正的品味是你的选择能被解释，而不是套了当下最安全的模板。",
