...
const TELLS = [
  ...
  { id: "12", group: "copy", name: "highlighted keywords", fix: "let structure carry emphasis",
    copy: true,
    patterns: [
      /<mark[\s>]/i,
      /text-(primary|indigo|purple|violet)-\d+/,
    ] },
  { id: "13", group: "copy", name: "AI copywriting voice", fix: "say the specific thing",
    copy: true,
    patterns: [
  ...
  { id: "14", group: "copy", name: "emoji everywhere", fix: "cut emoji from product copy",
    copy: true,
    patterns: [
      /\p{Extended_Pictographic}/u,
    ] },
  { id: "15", group: "component", name: "glowing status dot", fix: "flat dot + a word; no halo",
    patterns: [
      /\banimate-(?:ping|pulse)\b/i,
      /shadow-(?:green|emerald|lime)-\d+\/\d+/i,
      /(?:ready|online|live)[\s\S]{0,40}?(?:●|rounded-full)/i,
    ] },
  { id: "16", group: "component", name: "left-border callout", fix: "one aside, rest is body",
    patterns: [
      /border-l-4[\s\S]{0,40}?rounded|rounded[\s\S]{0,40}?border-l-4/i,
      /\b(admonition|callout|note-box)\b/i,
    ] },
  { id: "17", group: "component", name: "pastel icon tiles", fix: "labelled list with specifics",
    patterns: [
      /rounded-(lg|xl|2xl)\s+bg-(indigo|purple|blue|green|amber|pink)-(50|100)/i,
    ] },
  { id: "18", group: "component", name: "max-radius / glassmorphism", fix: "one small radius, solid surfaces",
    patterns: [
      /\brounded-full\b/,
      /backdrop-blur|backdrop-filter:\s*blur|bg-(?:white|black)\/(?:5|10|20|30)/i,
      /border-radius:\s*(?:9999px|50%|2rem|24px)/i,
    ] },
  { id: "19", group: "component", name: "oversized drop shadow", fix: "tight elevation, never bigger than the element",
    patterns: [
      /box-shadow:[^;{}]*\b(?:[6-9]\d|\d{3,})px/i,
      /shadow-\[[^\]]*\b(?:[6-9]\d|\d{3,})px/i,
      /filter:[^;{}]*drop-shadow\([^)]*\b(?:[6-9]\d|\d{3,})px/i,
    ] },
  { id: "20", group: "component", name: "corners that don't nest", fix: "inner = outer − padding",
    patterns: [
      /\brounded-(?:xl|2xl|3xl)\b/i,
      /border-radius:\s*(?:1rem|1\.5rem|24px|32px)/i,
    ] },
  { id: "21", group: "component", name: "badge / pill spam", fix: "badges only for real status",
    patterns: [
      /rounded-full[\s\S]{0,40}?bg-(indigo|purple|green|amber|pink)-(50|100|200)/i,
      />\s*(?:[✨🔥🎉🚀]\s*)?(new|beta|hot|popular|pro|coming soon)\s*</i,
    ] },
  { id: "22", group: "component", name: "AI-drawn SVG icon", fix: "a real, high-quality icon (a designer, or a strong image model)",
    patterns: [
      /<circle[^>]*\br="[1-9]/i,
      /(?:mascot|blob)\.svg/i,
    ] },
  { id: "23", group: "component", name: "icon in a tint of itself", fix: "no tinted tile; inherit text color",
    patterns: [
  ...
  { id: "24", group: "motion", name: "springy hover", fix: "transition what changes, 120–200ms, standard ease",
    patterns: [
  ...
  { id: "25", group: "layout", name: "all-caps card grid", fix: "show the one key thing fully",
    patterns: [
      /\bgrid-cols-3\b/,
      /\buppercase\b[\s\S]{0,30}?(?:text-xs|tracking-wide)/i,
      /\b(everything you need|why (?:you.?ll love|choose|teams))\b/i,
    ] },
  { id: "26", group: "layout", name: "invented stat row", fix: "only measured, sourced numbers",
    copy: true,
    patterns: [
  ...
  { id: "27", group: "layout", name: "01/02/03 section markers", fix: "number only real sequences",
    patterns: [
      /['"`>]0[1-9]['"`<]/,
      /text-[789]xl[\s\S]{0,50}?(?:text-(?:gray|slate|zinc|neutral)-(?:100|200)|opacity-(?:5|10|20))/i,
    ] },
  { id: "28", group: "layout", name: "cards inside cards", fix: "one surface per region; hairlines inside",
    patterns: [
      /<(Card|Panel|Box)[^>]*>\s*<\1\b/,
    ] },
  { id: "29", group: "evolved", name: "Inter everywhere", fix: "compare faces; be able to say why this one",
    patterns: [
  ...
  { id: "30", group: "evolved", name: "tasteful-terminal", fix: "mono for code only",
    patterns: [
      /\bfont-mono\b/,
