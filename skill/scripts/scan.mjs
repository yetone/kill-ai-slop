...
const TELLS = [
  ...
  { id: "22", group: "component", name: "badge / pill spam", fix: "badges only for real status",
    patterns: [
      /rounded-full[\s\S]{0,40}?bg-(indigo|purple|green|amber|pink)-(50|100|200)/i,
      />\s*(?:[✨🔥🎉🚀]\s*)?(new|beta|hot|popular|pro|coming soon)\s*</i,
    ] },
  { id: "23", group: "component", name: "AI-drawn SVG icon", fix: "a real, high-quality icon (a designer, or a strong image model)",
    patterns: [
      /<circle[^>]*\br="[1-9]/i,
      /(?:mascot|blob)\.svg/i,
    ] },
  { id: "24", group: "component", name: "icon in a tint of itself", fix: "no tinted tile; inherit text color",
    patterns: [
  ...
  { id: "25", group: "motion", name: "springy hover", fix: "transition what changes, 120–200ms, standard ease",
    patterns: [
  ...
  { id: "26", group: "layout", name: "all-caps card grid", fix: "show the one key thing fully",
    patterns: [
      /\bgrid-cols-3\b/,
      /\buppercase\b[\s\S]{0,30}?(?:text-xs|tracking-wide)/i,
      /\b(everything you need|why (?:you.?ll love|choose|teams))\b/i,
    ] },
  { id: "27", group: "layout", name: "invented stat row", fix: "only measured, sourced numbers",
    copy: true,
    patterns: [
  ...
  { id: "28", group: "layout", name: "01/02/03 section markers", fix: "number only real sequences",
    patterns: [
      /['"`>]0[1-9]['"`<]/,
      /text-[789]xl[\s\S]{0,50}?(?:text-(?:gray|slate|zinc|neutral)-(?:100|200)|opacity-(?:5|10|20))/i,
    ] },
  { id: "29", group: "layout", name: "cards inside cards", fix: "one surface per region; hairlines inside",
    patterns: [
      /<(Card|Panel|Box)[^>]*>\s*<\1\b/,
    ] },
  { id: "30", group: "layout", name: "one gap everywhere", fix: "space by relationship, not by token",
    patterns: [
      /(space-y-4|gap-4)\b[\s\S]{0,120}?\b(space-y-4|gap-4)\b/,
    ] },
  { id: "31", group: "evolved", name: "Inter everywhere", fix: "compare faces; be able to say why this one",
    patterns: [
  ...
  { id: "32", group: "evolved", name: "tasteful-terminal", fix: "mono for code only",
    patterns: [
      /\bfont-mono\b/,
